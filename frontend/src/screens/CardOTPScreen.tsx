...
type Props = {
  navigation: NativeStackNavigationProp<RootStackParamList, 'CardOTP'>;
  route: RouteProp<RootStackParamList, 'CardOTP'>;
};

const OTP_DURATION = 120;
const RESEND_DELAY = 30;

const CardOTPScreen: React.FC<Props> = ({ navigation, route }) => {
  ...
  const handleVerify = async () => {
    ...
    setLoading(true);
    try {
      await apiClient.post('/card/verify-otp', { otp });
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.title}>Confirm your card</Text>
      <Text style={styles.subtitle}>Enter the OTP sent by your bank to authorize this card.</Text>
